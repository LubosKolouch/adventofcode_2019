...
sub get_input {
    ...
    # ^ = 1
    if ( not defined $grid{ $robot_x - 1 }{$robot_y} ) {
        $robot_x -= 1;
        push @steps_q, 2;
        return 1;
    }
    ...
    # > = 3
    if ( not defined $grid{$robot_x}{ $robot_y + 1 } ) {
        $robot_y += 1;
        push @steps_q, 4;
        return 3;
    }
    ...
    return 0 if ( scalar @steps_q == 0 );
    my $direction = pop @steps_q;

    my %back_step_adj = ( 1 => ( -1, 0 ), 2 => ( 1, 0 ), 3 => ( 0, 1 ), 4 => ( 0, -1 ) );

    $robot_x -= 1 if $direction == 1;
    $robot_x += 1 if $direction == 2;
    $robot_y += 1 if $direction == 3;
    $robot_y -= 1 if $direction == 4;
...
sub process_output {
    ...
    my $response = shift @{ $processor{$cur_proc}{'io'} };
    ...
    if ( $response == 0 ) {
        ...
        $grid{$robot_x}{$robot_y} = ".";
        say( "oxygen found at " . $robot_x . " " . $robot_y );
        say scalar @steps_q;
...
# 9 - reset relative base
    ...
    while (1) {
        ...
        if ( $op == 1 ) {
            ...
            push @{ $processor{$cur_proc}{'io'} }, get_input($cur_proc);
...
for my $x ( keys %grid ) {
    for my $y ( keys %{ $grid{$x} } ) {
        ...
        # this is ugly but I needed it for debugging ;)
        my $vert1 = $x . ' ' . $y;
        ...
        $g->add_edge( $vert1, $x1edge ) if $grid{$x1}{$y} eq '.';
        $g->add_edge( $vert1, $x2edge ) if $grid{$x2}{$y} eq '.';
        $g->add_edge( $vert1, $x3edge ) if $grid{$x}{$y1} eq '.';
        $g->add_edge( $vert1, $x4edge ) if $grid{$x}{$y2} eq '.';
    }
}
...
for my $x ( keys %grid ) {
    for my $y ( keys %{ $grid{$x} } ) {
        ...
        my @path = $g->SP_Dijkstra( '14 16', $vert1);

        $max_path = scalar @path if scalar @path > $max_path;
    }
}
say $max_path-1;
