...
def run_intcode(cur_proc) :
    ...
    while 1:
        ...
        # Process the opcodes
            ...
            #instr = instr + str('10')
            print("providing ",inp)
            processor[cur_proc]['program'][ v1 ] = int(inp)
...
MAIN = [ord(ch) for ch in 'A,B,B,A,C,B,C,C,B,A\n']
X = [ord(ch) for ch in 'R,10,R,8,L,10,L,10\n']
Y = [ord(ch) for ch in 'R,8,L,6,L,6\n']
Z = [ord(ch) for ch in 'L,10,R,10,L,6\n']
CAM = [ord(ch) for ch in 'n\n']

inputs = MAIN + X + Y + Z + CAM

print(inputs)
inp = open(sys.argv[1]).read().strip().split(',')
data = list(map(int, inp))
data[0] = 2
out = run_program(data,1,inputs)
