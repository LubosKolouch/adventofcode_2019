...
def process_output(cur_proc) :
    qu = processor[cur_proc]['io']
    queue_length = len(qu)
    if (len(processor[cur_proc]['io']) == 2) :
        paint = processor[cur_proc]['io'].popleft()

        grid[(processor[cur_proc]['x'],processor[cur_proc]['y'])] = paint
        painted[(processor[cur_proc]['x'],processor[cur_proc]['y'])] = 1

        next_move = processor[cur_proc]['io'].popleft()
        if next_move == 0 :
            # ^ -> <
            if processor[cur_proc]['direction'] == '^': 
                processor[cur_proc]['direction'] = '<'
                processor[cur_proc]['x'] -= 1

            elif processor[cur_proc]['direction'] == '>': 
                processor[cur_proc]['direction'] = '^'
                processor[cur_proc]['y'] -= 1

            elif processor[cur_proc]['direction'] == 'v': 
                processor[cur_proc]['direction'] = '>'
                processor[cur_proc]['x'] += 1
